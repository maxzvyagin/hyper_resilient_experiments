...
train = tf.data.Dataset.from_generator(tf_gis_gen, args=(x_train, y_train), output_types=tf.float32).batch(config['batch_size'])
...
### this works on Theta: only problem is creating the tf Dataset is really slow
### in shell also need to do: export SM_FRAMEWORK=tf.keras to avoid keras.utils error
# import tensorflow as tf
# from hyper_resilient_experiments.segmentation.gis_preprocess import tf_gis_test_train_split
# from hyper_resilient_experiments.segmentation.UNet import TensorFlow_UNet_Model
# import segmentation_models as sm
